import { BEACON_VERSION, CommunicationClient, Serializer, Logger, WCStorage, SDK_VERSION } from '@airgap/beacon-core';
...
import { BeaconErrorType, BeaconMessageType, ExtendedWalletConnectPairingRequest, ExtendedWalletConnectPairingResponse, PermissionScope, StorageKey, TransportType } from '@airgap/beacon-types';
...
export class WalletConnectCommunicationClient extends CommunicationClient {
    ...
    async checkWalletReadiness(_topic) {
    ...
    async notifyListenersWithPermissionResponse(session, network, sessionEventId) {
        let publicKey;
    ...
    async init(forceNewConnection = false) {
        ...
        const { uri, approval } = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error('The connection timed out.')), 60000))
        ])
            .catch((error) => {
        ...
        approval()
            ...
            const pairingResponse = new ExtendedWalletConnectPairingResponse(session.topic, session.peer.metadata.name, session.peer.publicKey, '3', session.topic, session.peer.metadata.name);
    ...
    async updateActiveAccount(accounts, session) {
        try {
            if (accounts.length) {
    ...
    async onPairingClosed(signClient, topic) {
        const session = this.session?.pairingTopic === topic
            ? this.session
            : signClient.session
                .getAll()
    ...
    async closePairings() {
        ...
        pairings.length &&
            (await Promise.allSettled(pairings.map((pairing) => signClient.disconnect({
                topic: pairing.topic,
                reason: {
                    code: 0,
    ...
    async closeActiveSession(account, notify = true) {
        try {
    ...
    async notifyListeners(topic, partialResponse) {
        const response = {
            ...partialResponse,
            version: '2',
            senderId: topic
